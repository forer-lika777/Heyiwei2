...
你编写了一个宿舍管理服务 `DormManager`。包含学生和水费记录的添加、修改、删除等逻辑，开放`addStudent`、`removeStudent`、`addWaterRecord`等方法，外部函数调用前先调用`setDorm`设置要操作的对象，然后再调用以上方法，执行操作逻辑。

成员2：

你编写了一个主管理服务 `MainManager` 和一个工具类 `Utils`。主管理服务 `MainManager` 包含宿舍的添加、修改、删除等逻辑，开放`addDorm` `removeDorm` `updateDormInfo` 等方法，页面所有操作逻辑将调用 `MainManager` 中的方法完成数据管理操作。`MainManager` 持有一个 `DormManager` 对象，传入要操作的宿舍对象并调用其方法完成相应操作。工具类 `Utils` 开放多个静态方法供各个地方调用。

成员3：

你编写了一个水费记录管理服务 `RecordManager` 和数据保存服务 `DataSaveService`。水费记录管理服务 `RecordManager` 包含水费记录的添加、修改、删除等逻辑，开放 `addWaterRecord`、`removeWaterRecord` 等方法，供外部调用。数据保存服务 `DataSaveService` 包含 winrt 类型到普通 C++ 类型的转换以及 json 文件保存等逻辑，
