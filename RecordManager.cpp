...
//初始化RecordManager，传入水费记录和宿舍管理器的引用
RecordManager::RecordManager(std::vector<WaterRecord>& records, DormManager& dormMgr)
    : waterRecords(records), dormManager(dormMgr) {}
...
//根据学生ID、年月查询特定的水费记录，返回指向该记录的指针，如果未找到则返回nullptr
WaterRecord* RecordManager::querySpecificWaterRecord(const std::string& id, int year, int month) {
    for (auto& r : waterRecords) {
        if (r.id == id && r.year == year && r.month == month) {
            return &r;
        }
    }
    return nullptr;
